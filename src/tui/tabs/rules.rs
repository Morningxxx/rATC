...
pub fn render(f: &mut Frame<'_>, app: &App, _ui: &UiState, area: Rect) {
    let Some(sub) = &app.sub else {
        f.render_widget(Block::default().borders(Borders::ALL).title("路由规则"), area);
        return;
    };
    let items: Vec<ListItem> = sub.rules.iter().map(|r| {
        let line = match r {
    ...
    let title = format!("路由规则 成功{} 跳过{} 兜底{}", app.last_stats.rules_ok, app.last_stats.rules_skipped, app.last_stats.rules_fallback);
